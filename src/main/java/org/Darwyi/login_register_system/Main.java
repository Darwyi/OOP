...
public class Main {
    ...
    private static void showOrdered(UserRegistry registry, Scanner scanner) {
        ...
        switch (choice) {
            case "1":
                cmp = (a, b) -> Integer.compare(a.getId(), b.getId());
                break;
            case "2":
                cmp = (a, b) -> a.getName().compareToIgnoreCase(b.getName());
                break;
            case "3":
                cmp = (a, b) -> {
                ...
                break;
            default:
                System.out.println("Невідомий варіант");
                return;
        }
    ...
    private static void showFiltered(UserRegistry registry, Scanner scanner) {
        ...
        switch (choice) {
            case "1":
                pred = User::isLoggedIn;
                break;
            case "2":
                pred = u -> u.getLastLoginDate() == null;
                break;
            case "3":
                System.out.print("Літера: ");
                String letter = scanner.nextLine().trim().toLowerCase();
                pred = u -> !letter.isEmpty()
                        && u.getName().toLowerCase().startsWith(letter);
                break;
            default:
                System.out.println("Невідомий варіант");
                return;
        ...
        if (filtered.isEmpty()) {
            System.out.println("Жоден користувач не відповідає умові");
            return;
        }
